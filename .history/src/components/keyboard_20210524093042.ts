...
export default (state: KeyboardState, config: KeyboardConfig, action: Function) => {
  ...
  if (!state.layout) {
    const configList: any = config.layouts || {};
    const layouts = configList.layouts || [];
    const layoutRows = layout.rows.find((l: any) => l.name === config.layouts?.defaultLayout);
    state.layout = getLayoutTable(layoutRows);
  }
